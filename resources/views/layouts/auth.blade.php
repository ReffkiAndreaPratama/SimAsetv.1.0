...
<head>
    <meta charset="UTF-8">
    <title>@yield('title', 'SimAset') — SimAset RBTV Bengkulu</title>
    ...
    <style>
        *, *::before, *::after { font-family: 'Inter', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Page heading */
        .rp-heading { margin-bottom: 22px; }
        .rp-heading h2 { font-size: 1.3rem; font-weight: 800; color: #0F172A; margin-bottom: 4px; letter-spacing: -.025em; }
        .rp-heading p  { font-size: .8rem; color: #64748B; line-height: 1.6; }
        ...
        .fc-ro { background: #F1F5F9 !important; color: #64748B; cursor: not-allowed; }
        .pt { position: absolute; right: 11px; top: 50%; transform: translateY(-50%); color: #94A3B8; cursor: pointer; background: none; border: none; padding: 4px; font-size: 14px; line-height: 1; z-index: 1; }
        .pt:hover { color: #3B82F6; }

        /* Submit */
        .btn-go { width: 100%; padding: 12px 20px; background: linear-gradient(135deg, #1d4ed8, #3b82f6); color: #fff; border: none; border-radius: 9px; font-size: .875rem; font-weight: 700; cursor: pointer; transition: all .22s; display: flex; align-items: center; justify-content: center; gap: 8px; box-shadow: 0 4px 14px rgba(59,130,246,.25); font-family: 'Inter', sans-serif; letter-spacing: .01em; margin-top: 4px; }
        ...
        /* Back link */
        .back-link { display: flex; align-items: center; justify-content: center; gap: 6px; margin-top: 18px; font-size: .8rem; font-weight: 600; color: #64748B; text-decoration: none; transition: color .2s; }
        .back-link:hover { color: #2563EB; }

        /* Step indicator */
        .steps { display: flex; align-items: center; margin-bottom: 22px; }
        .step-item { display: flex; align-items: center; gap: 6px; }
        .step-num { width: 26px; height: 26px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: .68rem; font-weight: 700; flex-shrink: 0; }
        .step-num.active { background: #2563EB; color: #fff; box-shadow: 0 2px 8px rgba(37,99,235,.35); }
        .step-num.done   { background: #ECFDF5; border: 1.5px solid #6EE7B7; color: #059669; }
        .step-num.idle   { background: #F1F5F9; color: #CBD5E1; }
        .step-label { font-size: .72rem; font-weight: 600; white-space: nowrap; }
        ...
        /* Info box */
        .info-box { background: #F0F9FF; border: 1px solid #BAE6FD; border-radius: 9px; padding: 11px 13px; margin-bottom: 16px; font-size: .76rem; color: #0369A1; line-height: 1.65; }

        /* Password strength */
        .str-bar-wrap { height: 3px; background: #F1F5F9; border-radius: 4px; overflow: hidden; margin-top: 7px; }
        .str-bar { height: 100%; width: 0%; border-radius: 4px; transition: width .3s, background .3s; }
        .str-meta { display: flex; justify-content: space-between; margin-top: 4px; }
        .str-text { font-size: .68rem; }
        .str-hint { font-size: .68rem; color: #CBD5E1; }

        /* Password rules */
        .pw-rules { background: #F8FAFC; border: 1px solid #F1F5F9; border-radius: 9px; padding: 11px 13px; margin-bottom: 16px; }
        .pw-rules-title { font-size: .7rem; font-weight: 600; color: #374151; margin-bottom: 8px; }
        .pw-rules-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 5px 14px; }
        .pw-rule { font-size: .7rem; color: #CBD5E1; display: flex; align-items: center; gap: 6px; transition: color .2s; }
        .pw-rule::before { content: ''; width: 5px; height: 5px; border-radius: 50%; background: currentColor; flex-shrink: 0; }
        .pw-rule.ok { color: #059669; }

        /* Match msg */
        .match-msg { font-size: .68rem; margin-top: 4px; min-height: 16px; }

        .rp-footer { text-align: center; margin-top: 22px; font-size: .65rem; color: #CBD5E1; }
