...
<head>
    <meta charset="UTF-8">
    <title>Laporan Aset — RBTV Bengkulu</title>
    <style>
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#1a3470;margin-bottom:0;">
    ...
    <tr>
        <td align="center" style="padding:8px 16px 10px;border-top:1px solid rgba(255,255,255,0.15);">
            <div style="font-size:13px;font-weight:900;color:#fff;text-transform:uppercase;letter-spacing:0.08em;">LAPORAN DATA ASET BARANG KANTOR</div>
            <div style="font-size:7.5px;color:rgba(255,255,255,0.6);margin-top:2px;">Dokumen resmi inventarisasi aset &mdash; dicetak oleh sistem SimAset</div>
        </td>
    </tr>
</table>
...
<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:10px;">
    <tr><td height="3" style="background:#b91c1c;font-size:0;line-height:0;">&nbsp;</td></tr>
</table>
...
@php
    ...
    $nonaktif    = $assets->where('status', 'Non-Aktif')->count();
    $baik        = $assets->where('kondisi', 'Baik')->count();
    $rusak       = $assets->whereIn('kondisi', ['Rusak Ringan', 'Rusak Berat'])->count();
@endphp
<table width="100%" cellpadding="0" cellspacing="0"
       style="border:1px solid #e2e8f0;border-collapse:collapse;margin-bottom:10px;">
    <tr>
        <td width="16.6%" align="center" style="padding:7px 4px;border-right:1px solid #e2e8f0;">
            <span style="font-size:7px;color:#94a3b8;text-transform:uppercase;display:block;">Total Aset</span>
        ...
        <td width="16.6%" align="center" style="padding:7px 4px;border-right:1px solid #e2e8f0;">
            ...
            <span style="font-size:16px;font-weight:900;color:#059669;display:block;line-height:1.2;">{{ $aktif }}</span>
        </td>
        <td width="16.6%" align="center" style="padding:7px 4px;border-right:1px solid #e2e8f0;">
            <span style="font-size:7px;color:#94a3b8;text-transform:uppercase;display:block;">Maintenance</span>
            <span style="font-size:16px;font-weight:900;color:#0e7490;display:block;line-height:1.2;">{{ $maintenance }}</span>
        </td>
        <td width="16.6%" align="center" style="padding:7px 4px;border-right:1px solid #e2e8f0;">
            <span style="font-size:7px;color:#94a3b8;text-transform:uppercase;display:block;">Non-Aktif</span>
            <span style="font-size:16px;font-weight:900;color:#64748b;display:block;line-height:1.2;">{{ $nonaktif }}</span>
        </td>
        <td width="16.6%" align="center" style="padding:7px 4px;border-right:1px solid #e2e8f0;">
        ...
        <td width="16.6%" align="center" style="padding:7px 4px;">
            <span style="font-size:7px;color:#94a3b8;text-transform:uppercase;display:block;">Perlu Perhatian</span>
            <span style="font-size:16px;font-weight:900;color:#dc2626;display:block;line-height:1.2;">{{ $rusak }}</span>
        </td>
    </tr>
</table>
...
<table width="100%" cellpadding="0" cellspacing="0"
    ...
    <tr>
        <td style="padding:5px 12px;font-size:8px;color:#64748b;">
            ...
            &nbsp;&nbsp;&nbsp;
            <strong style="color:#1e293b;">Total:</strong> {{ $total }} aset
        </td>
    </tr>
</table>
...
<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:12px;">
    <thead>
        <tr>
            ...
            <th width="75" style="background:#1a3470;color:#fff;padding:6px 5px;font-size:7.5px;font-weight:700;text-transform:uppercase;border:1px solid #1a3470;">Kategori</th>
            <th width="80" style="background:#1a3470;color:#fff;padding:6px 5px;font-size:7.5px;font-weight:700;text-transform:uppercase;border:1px solid #1a3470;">Ruangan</th>
            <th width="60" align="center" style="background:#1a3470;color:#fff;padding:6px 5px;font-size:7.5px;font-weight:700;text-transform:uppercase;border:1px solid #1a3470;">Kondisi</th>
            <th width="55" align="center" style="background:#1a3470;color:#fff;padding:6px 5px;font-size:7.5px;font-weight:700;text-transform:uppercase;border:1px solid #1a3470;">Status</th>
    ...
    <tbody>
        @foreach($assets as $a)
        @php $rowBg = $loop->even ? '#f8fafc' : '#ffffff'; @endphp
        <tr>
            ...
            <td style="padding:5px;border:1px solid #e2e8f0;background:{{ $rowBg }};font-size:7.5px;color:#64748b;">{{ $a->keterangan ?? '—' }}</td>
        </tr>
        @endforeach
    </tbody>
    <tfoot>
