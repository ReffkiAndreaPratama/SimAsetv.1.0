"""
Perbaikan ACT-06 QR Scanner, ACT-09 Audit Log, ACT-10 Maintenance
"""
import os, math
...
def arr_h_r(d,x1,y,x2,lbl=""):
    d.line([(x1,y),(x2,y)],fill=BLACK,width=1)
    d.polygon([(x2,y),(x2-8,y-4),(x2-8,y+4)],fill=BLACK)
    if lbl:
        bb=FS.getbbox(lbl); tw=bb[2]-bb[0]
        d.text(((x1+x2)//2-tw//2,y-FS.size-2),lbl,font=FS,fill=GRAY)

def arr_h_l(d,x1,y,x2,lbl=""):
    d.line([(x1,y),(x2,y)],fill=BLACK,width=1)
    d.polygon([(x2,y),(x2+8,y-4),(x2+8,y+4)],fill=BLACK)
    if lbl:
        bb=FS.getbbox(lbl); tw=bb[2]-bb[0]
...
class Act:
    ...
    def __init__(self,title,l1,l2,w1=300,w2=400):
...
def act_qr():
    ...
    g.cross(0,1)
    g.act(1,"Menampilkan halaman scanner\nMinta izin kamera browser")
    g.ret(1,0)
    g.act(0,"Izinkan akses kamera")
    g.act(0,"Arahkan kamera ke\nQR Code aset fisik")
    g.cross(0,1)
    g.act(1,"Decode QR Code\nParse kode_aset dari URL")
    g.act(1,"GET /aset/{kode}/detail\n[route PUBLIK]")
    g.dec(1,"Aset\nditemukan?","Ya","Tidak - 404")
    g.act(1,"Tampilkan detail aset:\nNama, Kategori, Ruangan,\nKondisi, Status, Serial")
    g.ret(1,0)
    g.act(0,"Melihat detail aset")
    g.end()
...
def act_auditlog():
    g=Act("act  Audit Log (Admin Only) — SimAset","Admin","Sistem",300,400)
    g.start()
    g.act(0,'Klik menu "Log Aktivitas"')
    g.cross(0,1)
    g.act(1,"Cek middleware role:admin")
    g.dec(1,"Role =\nadmin?","Ya","Tidak - 403")
    g.act(1,"ActivityLog::with('user')\n->orderBy('created_at','desc')\n->paginate(20)")
    g.ret(1,0)
    g.act(0,"Tampilkan tabel log aktivitas")
    g.act(0,"Atur filter (opsional):\n- Kata kunci\n- Pengguna\n- Modul\nKlik Filter")
    g.cross(0,1)
    g.act(1,"Query dengan WHERE clause\nTampilkan hasil (paginated)")
    g.ret(1,0)
    g.act(0,"Melihat riwayat aktivitas")
    g.end()
...
def act_maintenance():
    ...
    g.ret(1,0)
    g.act(0,"Aset berhasil ditandai Maintenance")
