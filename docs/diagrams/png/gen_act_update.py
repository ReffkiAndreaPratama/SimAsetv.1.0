"""
Update ACT_02 Aset dan ACT_03 Barang:
- Import & Export masuk ke fork aksi masing-masing
- Tidak ada diagram Import/Export terpisah
"""
import os, math
...
# ============================================================
# ACT-02 MANAJEMEN ASET — termasuk Import & Export
# ============================================================
def act_aset():
    g=Act("act  Manajemen Aset — SimAset","Pengguna","Sistem",300,420)
    g.start()
    ...
    g.fork(0,["Tambah\nAset","Lihat\nAset","Edit\nAset","Hapus\nAset",
              "Generate\nQR","Import\nAset","Export\nAset"])
    g.cross(0,1)
    g.act(1,"Validasi & Proses Input")
    g.dec(1,"Berhasil?","Ya","Tidak")
    g.act(1,"Simpan ke Database\nLog Aktivitas")
    g.ret(1,0)
    g.act(0,"Selesai")
    g.end()
...
# ============================================================
# ACT-03 MANAJEMEN BARANG — termasuk Import & Export
# ============================================================
def act_barang():
    g=Act("act  Manajemen Barang — SimAset","Pengguna","Sistem",300,420)
    g.start()
    g.act(0,'Pilih menu "Barang"')
    g.cross(0,1)
    g.act(1,"Query Barang\nTampilkan daftar + filter")
    g.ret(1,0)
    g.fork(0,["Tambah\nBarang","Lihat\nBarang","Edit\nBarang","Hapus\nBarang",
              "Import\nBarang","Export\nBarang"])
    g.cross(0,1)
